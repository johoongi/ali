...
for ship_chk in all_shipping_charges[0:]:
    if 'AliExpress Standard Shipping' not in ship_chk.text:
            chuga_button = driver.find_element(By.CSS_SELECTOR, 'button.comet-btn comet-btn-text _3djqy')
            if chuga_button:
                      print('추가 클릭')
...
for ship in all_shipping_charges[0:]:
    if 'AliExpress Standard Shipping' in ship.text:
        #print('Standard Shipping')
        ship_pice_text = ship.find_element(By.CSS_SELECTOR, 'strong').text
...
##옵션
option_all = ''
for option in driver.find_elements(By.CSS_SELECTOR, 'div.sku-title'):
    option_all = option_all + option.text + '</br>'
...
for option_image in optional_image_all[0:]:
    ...
    ##상품가격 
    price_origin = driver.find_element(By.CSS_SELECTOR, 'span.product-price-value').text
    #print(price_origin)
